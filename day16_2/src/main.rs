...
fn run(mapper: &HashMap<usize, &&str>, opcode: &[usize], registers: &[usize]) -> [usize; 4] {
    match mapper.get(&opcode[0]).unwrap() {
        &&"addr" => addr(opcode, registers),
        &&"addi" => addi(opcode, registers),
        &&"mulr" => mulr(opcode, registers),
        &&"muli" => muli(opcode, registers),
        &&"banr" => banr(opcode, registers),
        &&"bani" => bani(opcode, registers),
        &&"borr" => borr(opcode, registers),
        &&"bori" => bori(opcode, registers),
        &&"setr" => setr(opcode, registers),
        &&"seti" => seti(opcode, registers),
        &&"gtir" => gtir(opcode, registers),
        &&"gtri" => gtri(opcode, registers),
        &&"gtrr" => gtrr(opcode, registers),
        &&"eqir" => eqir(opcode, registers),
        &&"eqri" => eqri(opcode, registers),
        &&"eqrr" => eqrr(opcode, registers),
        _ => unreachable!(),
    }
}
#[aoc(2018, 16, 2)]
fn main(input: &str) -> usize {
        ...
        for (code, names) in map.iter() {
            if names.len() == 1 {
                for name in names.iter() {
                    for instruction in newmap.values_mut() {
                        instruction.remove(name);
                    }
                    finals.insert(*code, name.clone());
                }
            }
        }
    ...
    // hehehe, I cheated here.
    for line in input.lines().skip(3138) {
        let mut instruction = [0; 4];

        for (src, dst) in line.split(' ').zip(instruction.iter_mut()) {
            *dst = src.parse().unwrap();
        }

        registers = run(&finals, &instruction, &registers);
    }
